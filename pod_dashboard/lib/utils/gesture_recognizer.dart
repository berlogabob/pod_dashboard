...
class GestureRecognizer {
  GestureStatus recognize(List<Landmark> landmarks, int transformMode) {
    ...
    final middleDist =
        (middleTip.y - middleMcp.y).abs() + (middleTip.x - middleMcp.x).abs();
    final ringDist =
        (ringTip.y - ringMcp.y).abs() + (ringTip.x - ringMcp.x).abs();
    final pinkyDist =
        (pinkyTip.y - pinkyMcp.y).abs() + (pinkyTip.x - pinkyMcp.x).abs();
    ...
    if (thumbLength < GestureThresholds.minThumbLength)
